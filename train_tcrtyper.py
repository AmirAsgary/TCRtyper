...
def parse_args():
    ...
    p.add_argument("--lr_schedule_restart", action="store_true",
                   default=False, help="If enabled, restart cosine decay enabled, else, no restart")
    ...
    p.add_argument("--cold_start", action="store_true", default=False,
                   help="If enabled shift hla_log_odds so the MOST common allele starts at γ ≈ 0.01")
    # ── tokens ───────────────────────────────────────────────────────
...
def load_hla_embeddings_for_clip(args, idx_to_hla):
    ...
    Unlike the old code which transposed and SVD-projected to get a
    Dense(A) kernel, here we keep the embeddings as (A, esm_dim) and
    let the CLIPBindingHead's internal Dense handle the projection.
    ...
    if not args.hla_embed or not os.path.exists(args.hla_embed):
        print("[DATA] No HLA embeddings provided — CLIP head uses random init")
        return None
    ...
    # embed_matrix shape: (A, esm_dim) — keep as-is for CLIP head
    # The CLIPBindingHead will project (A, esm_dim) → (A, clip_dim) internally
...
def build_model(args,
                ...
                hla_bias_init=None):
    """
    Build the TCRtyper Keras functional model with CLIP-style dual encoder.
    ...
    Architecture:
      TCR path:
        CDR freq (B,L,21) + mask (B,L)
          ...
          → Dense(clip_dim, gelu)        → (B, D)    ← TCR embedding
          → Dropout
    
      HLA path (inside CLIPBindingHead):
        ESM embeddings (A, 32)
          → Dense(clip_dim)              → (A, D)    ← HLA embedding
    
      Binding:
        z_logits = tcr_embed @ hla_embed^T + bias    → (B, A)
    
    Parameters
    ----------
    args : argparse.Namespace
        Must contain: embed_dim, num_heads, num_layers, ff_dim, hla_proj_dim,
        max_seq_len, encoding_mode, dropout, num_alleles, train_hla_head,
        and all token values (pad_token, mask_token, sep_token, normal_token).
    hla_embed_matrix : np.ndarray or None, shape (A, hla_input_dim)
        Pre-computed HLA embeddings from ESM (NOT transposed).
        If None, random initialisation is used.
    hla_bias_init : np.ndarray or None, shape (A,)
        Log-odds bias: ln(f_a / (1 - f_a)).
    
    Returns
    -------
    keras.Model with inputs [seq_input, mask_input] and output z_logits (B, A).
    """
    ff_dim = args.ff_dim if args.ff_dim > 0 else args.embed_dim * 4
    clip_dim = args.hla_proj_dim  # shared embedding dimension

    # ── inputs ───────────────────────────────────────────────────────
    inp_seq = tf.keras.Input(shape=(None, 21), name="seq_input")
    inp_mask = tf.keras.Input(shape=(None,), dtype=tf.int32, name="mask_input")
    ...
    # Project TCR into the shared CLIP space: (B, embed_dim) → (B, clip_dim)
    tcr_embed = layers.Dense(
        clip_dim, activation="gelu", name="tcr_clip_proj", kernel_initializer=tf.keras.initializers.GlorotUniform()
    )(h)
    tcr_embed = layers.Dropout(args.dropout, name="tcr_clip_drop")(tcr_embed)

    # ── CLIP binding head (NN_φ + dot product + bias) ────────────────
    # Determine HLA input dimensionality from the provided embeddings
    if hla_embed_matrix is not None:
        hla_input_dim = hla_embed_matrix.shape[1]  # (A, 32) → 32
    else:
        hla_input_dim = clip_dim  # fallback: random init in clip_dim

    z_logits = CLIPBindingHead(
        num_alleles=args.num_alleles,
        clip_dim=clip_dim,
        hla_input_dim=hla_input_dim,
        hla_embed_init=hla_embed_matrix,   # (A, 32) or None
        bias_init=hla_bias_init,           # (A,) log-odds or None
...
class WarmupCosineDecayRestarts(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    After the warmup, the LR decays to min_lr over `cycle_steps`.
    Once a cycle is complete, it instantly restarts at `peak_lr` and decays again.
    """
    ...
    def __call__(self, step):
        ...
        # 2. SGDR phase (happens after warmup)
        # Shift step so the first cycle starts at 0 after warmup
        step_after_warmup = tf.maximum(step - self.warmup_steps, 0.0)
        
        # Modulo arithmetic to find where we are in the current cycle
        step_in_cycle = tf.math.mod(step_after_warmup, self.cycle_steps)
        progress = step_in_cycle / self.cycle_steps
        
        # Standard cosine decay formula applied to the current cycle
        cosine_lr = self.min_lr + 0.5 * (self.peak_lr - self.min_lr) * (
            1.0 + tf.cos(math.pi * progress))
...
def train_step_lean(model, loss_fn, optimizer,
                    ...
                    grad_clip):
    """Fast training step: forward + backward + apply gradients.
    No diagnostics — used for the majority of steps where we only
    need loss values for accumulation.
    Supports mixed-precision via optimizer.get_scaled_loss.
        Args:
        combined_cdr:   (B, L, 21) float32
        combined_mask:  (B, L)     int32
        binder_dense:   (B, A)     float32
    ...
    # Mixed-precision: unscale gradients back to float32
    if hasattr(optimizer, "get_scaled_loss"):
...
def train_step_with_diag(model, loss_fn, optimizer,
                         combined_cdr, combined_mask, binder_dense, donor_indices,
                         grad_clip):
    """Full training step with diagnostics — called only at log intervals.
    Identical gradient computation with mixed-precision support,
    but also runs compute_diagnostics for TensorBoard and console logging.
        Args:
        combined_cdr:   (B, L, 21) float32
        combined_mask:  (B, L)     int32
        binder_dense:   (B, A)     float32
...
def compute_diagnostics(z_logits, binder_dense, model=None):
    ...
    # ── METRICS FOR ACTIVE (CO-OCCURRING) ALLELES ──
    ...
    max_gamma_per_sample = tf.reduce_max(tf.sigmoid(masked_logits), axis=-1)
    mean_max_gamma = tf.reduce_mean(max_gamma_per_sample)
    
    p_safe = tf.clip_by_value(z_prob, eps, 1.0 - eps)
    ent = -(p_safe * tf.math.log(p_safe) + (1.0 - p_safe) * tf.math.log(1.0 - p_safe))
    ...
    # ── METRICS FOR INACTIVE (NON-CO-OCCURRING) ALLELES ──
    mean_inactive_prob = tf.reduce_sum(z_prob * inactive_mask) / tf.maximum(n_inactive, 1.0)
    max_inactive_per_sample = tf.reduce_max(z_prob * inactive_mask + active_mask * -1e9, axis=-1)
    mean_max_inactive_prob = tf.reduce_mean(max_inactive_per_sample)
    min_inactive_per_sample = tf.reduce_min(z_prob + active_mask * 1e9, axis=-1)
    mean_min_inactive_prob = tf.reduce_mean(min_inactive_per_sample)
    
    # ── DIVERSITY / MODE COLLAPSE TRACKING ──
    # 1. Setup XLA-safe shape handling
    num_alleles = z_logits.shape[-1]
    if num_alleles is None:
        num_alleles = args.num_alleles  # Fallback safety for XLA
        
    batch_size_float = tf.cast(tf.shape(z_logits)[0], tf.float32)

    # 2. ACTIVE ONLY (What it chooses among alleles the donor actually has)
    top1_active_indices = tf.argmax(masked_logits, axis=-1) 
    active_counts = tf.reduce_sum(tf.one_hot(top1_active_indices, depth=num_alleles), axis=0)
    num_unique_top1_active = tf.reduce_sum(tf.cast(active_counts > 0, tf.float32))
    max_batch_fraction_active = tf.reduce_max(active_counts) / tf.maximum(batch_size_float, 1.0)

    # 3. ALL HLAs (The true raw preference of the Neural Network)
    top1_global_indices = tf.argmax(z_logits, axis=-1) 
    global_counts = tf.reduce_sum(tf.one_hot(top1_global_indices, depth=num_alleles), axis=0)
    num_unique_top1_global = tf.reduce_sum(tf.cast(global_counts > 0, tf.float32))
    max_batch_fraction_global = tf.reduce_max(global_counts) / tf.maximum(batch_size_float, 1.0)

    # ── TOP-5 GLOBAL ALLELES (by mean batch z_prob) ──
    mean_prob_per_allele = tf.reduce_mean(z_prob, axis=0)                         # (A,)
    top5g_prob, top5g_idx = tf.math.top_k(mean_prob_per_allele, k=5)
    active_frac_per_allele = tf.reduce_mean(active_mask, axis=0)                  # (A,)
    top5g_frac = tf.gather(active_frac_per_allele, top5g_idx)
    # ── TOP-5 ACTIVE ALLELES (by mean prob among samples where active) ──
    active_count = tf.reduce_sum(active_mask, axis=0)                             # (A,)
    mean_prob_where_active = tf.reduce_sum(z_prob * active_mask, axis=0) / tf.maximum(active_count, 1.0)
    top5a_prob, top5a_idx = tf.math.top_k(mean_prob_where_active, k=5)
    top5a_frac = tf.gather(active_count, top5a_idx) / tf.maximum(batch_size_float, 1.0)

    if model is not None:
        temperature = tf.exp(model.get_layer("clip_binding_head").log_temperature)
    return {
        ...
        "max_batch_frac_active": max_batch_fraction_active,
...
def run_epoch(model, loss_fn, optimizer, h5_path, args,
    ...
    while True:
        ...
        if is_train:
            # Only compute expensive diagnostics at log intervals
...
def run_epoch_tfrecord(model, loss_fn, optimizer, dataset, args,
    ...
    for cdr, mask, binder, donors in dataset:
        ...
        n_steps += 1
        # Track whether this step has diagnostics
        has_diag = "mean_active_gamma" in metrics
        if has_diag:
            n_diag_steps += 1
        for k, v in metrics.items():
...
def run_inference_tfrecord(model, tfrecord_dir, output_path, args):
    """
    Run inference on pre-built TFRecord shards and save results to NPZ.
    Reads shards sequentially (no interleave) to preserve cluster order.
    Saves z_logits, z_probs, binder_dense, and donor_indices.
    Args:
        model:        compiled Keras model
        tfrecord_dir: path to directory containing *.tfrecord shard files
    ...
    print(f"{'='*60}")
    # ── discover shard files in the directory ────────────────────────
    shard_paths = sorted([
    ...
    # ── check manifest for cluster count ─────────────────────────────
    ...
    if os.path.exists(manifest_path):
        ...
        print(f"  Manifest reports {n_clusters_est} clusters")
    # ── parse function ───────────────────────────────────────────────
    num_alleles = args.num_alleles
    def _parse(example_proto):
        """Parse one TFRecord Example into all stored tensors."""
        desc = {
            "combined_cdr_flat": tf.io.VarLenFeature(tf.float32),
            "combined_mask":     tf.io.VarLenFeature(tf.int64),
            "binder_dense":      tf.io.FixedLenFeature([num_alleles], tf.float32),
        ...
        donors = tf.cast(tf.sparse.to_dense(parsed["donor_indices"]), tf.int32)
        return cdr, mask, binder, donors
    # ── sequential read for deterministic ordering ───────────────────
    dataset = tf.data.TFRecordDataset(shard_paths)
    dataset = dataset.map(_parse, num_parallel_calls=tf.data.AUTOTUNE)
    # ── pad and batch (no shuffle, no masking) ───────────────────────
    ...
    # ── forward pass ─────────────────────────────────────────────────
    from tqdm import tqdm
    total_steps = math.ceil(n_clusters_est / args.batch_size) if n_clusters_est else None
    all_logits = []
    all_probs = []
    all_binders = []
    all_donors = []
    n_processed = 0
    max_donor_len = 0
    for cdr, mask, binder, donors in tqdm(dataset, total=total_steps,
                                           desc="Inferring z_probs",
                                           unit="batch"):
        # Forward pass (no gradient, no dropout)
        ...
        # Track donor indices and max length for later padding
    ...
    # ── concatenate results ──────────────────────────────────────────
    if n_processed == 0:
        print("ERROR: 0 clusters processed — check TFRecord files")
        sys.exit(1)
    ...
    donor_unified = np.full((n_processed, max_donor_len),
                            args.pad_token, dtype=np.int32)
    offset = 0
    for d in all_donors:
        n, cols = d.shape
        donor_unified[offset:offset + n, :cols] = d
        offset += n
    # ── derive n_donors from donor_indices ───────────────────────────
    n_donors_all = np.sum(donor_unified != args.pad_token, axis=1).astype(np.int32)
    # ── save ─────────────────────────────────────────────────────────
    np.savez_compressed(
        output_path,
        z_logits=z_logits_all,
        z_probs=z_probs_all,
    ...
    print(f"  z_probs       shape: {z_probs_all.shape}")
    print(f"  binder_dense  shape: {binder_all.shape}")
    print(f"  donor_indices shape: {donor_unified.shape}")
    print(f"  n_donors      shape: {n_donors_all.shape}")
    print(f"  ✓ Inference complete → {output_path}")
# ═════════════════════════════════════════════════════════════════════
# 11. MAIN
# ═════════════════════════════════════════════════════════════════════
def main():
    ...
    if args.cold_start:
        # Shift so the MOST common allele starts at γ ≈ 0.01
        shift = hla_log_odds.max() + 4.6  # sigmoid(-4.6) ≈ 0.01
    ...
    if args.hla_embed and os.path.exists(args.hla_embed):
        ...
        hla_embed_matrix = load_hla_embeddings_for_clip(args, idx_to_hla)
    ...
    if args.mode == "train":
        ...
        steps_per_epoch = 1  # placeholder for inference/export
    ...
    # ════════════════════════════════════════════════════════════════
        ...
        if args.use_tfrecord:
            if not args.tf_record_path:
                print("ERROR: --tf_record_path is required for TFRecord "
                      "inference (path to shard directory)")
                sys.exit(1)
            if not os.path.isdir(args.tf_record_path):
                print(f"ERROR: {args.tf_record_path} is not a directory")
                sys.exit(1)
            output_npz = os.path.join(args.output_dir, "predictions.npz")
            run_inference_tfrecord(
                model, args.tf_record_path, output_npz, args)
        # ── H5-based inference (original path) ───────────────────────
        else:
