#!/usr/bin/env python3
"""
MLE Pipeline for Real TCR-HLA Data (Dense / NonSparseTCRModel variant)
— with checkpointing and parallel support.

This is the dense counterpart of mle_real_data.py. It uses NonSparseTCRModel
which evaluates ALL HLA alleles per TCR (no candidate masking via binder_sets).
...
The checkpoint format is IDENTICAL to the sparse version: z_probs are projected
back onto binder_sets before saving so that the merge step and downstream
...
=== PARALLEL USAGE ===
  ...
  python mle_real_data_dense.py --h5_data_path data.h5 --donor_matrix_path donor.npz \\
  ...
  python mle_real_data_dense.py ... --mode train --chunk_range 0 10

  # After all jobs finish, merge:
  python mle_real_data_dense.py ... --mode merge
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MLE training on real TCR-HLA data — Dense (NonSparseTCRModel), "
                    "chunk-wise, parallel-safe.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Sequential with resume:
  python mle_real_data_dense.py --h5_data_path data.h5 --donor_matrix_path donor.npz --output_dir /out --mode train --resume

  # Single chunk (for cluster array jobs):
  python mle_real_data_dense.py ... --mode train --chunk_id 5
  ...
  # Check progress:
  python mle_real_data_dense.py ... --mode status
        """,
    )
...
def extract_z_probs_from_dense_model(
    model, binder_sets: np.ndarray, pad_token: float
) -> np.ndarray:
    """Extract z_probs from NonSparseTCRModel and project onto binder_sets.

    The dense model has an embedding of shape (num_tcrs, num_total_alleles).
    We apply sigmoid to get the full probability matrix, then gather only the
    columns indicated by binder_sets for each TCR so that the output has the
    same shape and meaning as the sparse model's z_probs:
        (num_tcrs, max_hlas_per_tcr)
    Padded positions (where binder_sets == pad_token) are set to 0.

    This keeps the checkpoint format identical to mle_real_data.py so that
    the merge step and all downstream readers work unchanged.
    """
    z_logits = model.z_embedding.get_weights()[0]  # (num_tcrs, num_total_alleles)
    full_probs = tf.sigmoid(z_logits).numpy().astype(np.float32)

    # Gather candidate allele probs: use 0 for padded positions (value irrelevant)
    safe_indices = np.maximum(binder_sets, 0).astype(np.int64)
    z_probs_sparse = np.take_along_axis(full_probs, safe_indices, axis=1)

    # Zero out padded positions
    z_probs_sparse[binder_sets == pad_token] = 0.0
    return z_probs_sparse
...
def run_train(args):
    ...
    from utils import (
        NonSparseTCRModel, create_dataset, pad_list_to_array_without_max,
    ...
    print(f"  Model: NonSparseTCRModel (dense, {num_alleles} alleles)")
    ...
    with PublicTcrHlaCsrReaderChunk(
        ...
        for chunk in reader.iter_cluster_chunks(
            ...
            # Binder sets: nonzero allele indices (needed for checkpoint format)
            ...
            print(f"  binder_sets: ({n_clusters}, {max_hlas}) | "
                  ...
                  f"dense embedding: ({n_clusters}, {num_alleles})")
            ...
            # Model — NonSparseTCRModel (dense: all alleles)
            ...
            # Extract z_probs — project dense (n, A) back to sparse (n, max_hlas)
            z_probs = extract_z_probs_from_dense_model(
                model, binder_sets, args.pad_token
            )

            # Save checkpoint (same format as sparse pipeline)
...
def run_status(args):
    ...
    print(f"\n{'='*60}")
    print(f"MLE Pipeline Status (Dense / NonSparseTCRModel)")
    print(f"{'='*60}")
    ...
    if missing:
        ...
        print(f"    python mle_real_data_dense.py \\")
